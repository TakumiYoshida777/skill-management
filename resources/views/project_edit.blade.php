...
@section('content')
    ...
    <form action="{{ url('project', $projectData->id) }}" method="POST">
        @csrf
        @method('PUT')
        <div class="btns-wrap">
            <div class="pc">
        ...
        <div class="inner-heding mb-4 row">
            ...
            <div class="project-title col-md-6">
                <input type="text" name="name" id="name" class="form-control" placeholder="新規プロジェクト名" required
                    value="{{ old('name', $projectData->name) }}">
            </div>
            {{-- 期間 --}}
            <div id="duration" class="grid-container col-md-6">
                <div class="date-grid px-2">
                    <div class="date-title">開始日</div>
                    <div class="date-item ml-1"><input type="date" class="form-control" name="start_date" required
                            value="{{ old('start_date',$projectData->start_date) }}"></div>
                    <div class="date-title">終了日</div>
                    <div class="date-item ml-1"><input type="date" class="form-control" name="end_date"
                            value="{{ old('end_date',$projectData->end_date) }}"></div>
                </div>
            </div>
        </div>
            ...
            <div class="inner-container">
                ...
                <div id="project-description" class="grid-container card w-100">
                    ...
                    <div class="grid-body p-2">
                        <textarea class="form-control" name="description" required>{{ old('description',$projectData->description) }}</textarea>
                    </div>
                </div>
                {{-- チーム情報 --}}
                <div id="team-info" class="grid-container card w-100">
                    ...
                    <div class="grid-body px-2">
                        <div class="grid-record">
                            ...
                            <div class="grid-ritem"><input type="text" class="form-control" name="position"
                                    value="{{ old('position',$projectData->position) }}"></div>
                        </div>
                        <div class="grid-record">
                            <div class="grid-ritem">チーム人数</div>
                            <div class="grid-ritem"><input type="number" class="form-control" name="team_size"
                                    min="1" value="{{ old('team_size',$projectData->team_size) }}">
                            </div>
                        </div>
                        <div class="grid-record">
                            <div class="grid-ritem">全体人数</div>
                            <div class="grid-ritem"><input type="number" class="form-control" name="all_team_size"
                                    min="1" value="{{ old('all_team_size',$projectData->all_team_size) }}">
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="grid-body px-2 ">
                        ...
                        <div class="grid-record">
                            ...
                            <div class="grid-ritem"><input type="checkbox" class="form-control" name="programming_flag"
                                    @if ($projectData->programming_flag == 1) checked @endif>
                            </div>
                        </div>
                        <div class="grid-record">
                        ...
                        <div class="grid-record">
                            ...
                            <div class="grid-ritem"><input type="checkbox" class="form-control"
                                    name="system_migration_flag" @if ($projectData->system_migration_flag == 1) checked @endif></div>
                        </div>
                        <div class="grid-record">
                            <div class="grid-ritem">運用・保守</div>
                            <div class="grid-ritem"><input type="checkbox" class="form-control"
                                    name="operation_maintenance_flag" @if ($projectData->operation_maintenance_flag == 1) checked @endif>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="inner-container">
                ...
                <div id="framework" class="grid-container card w-100">
                    ...
                    <div class="grid-body px-2 ">
                        ...
                        @foreach ($used_frameworks as $index => $used_data)
                            <div class="grid-record" data-recordid="{{ $index }}">
                                <div class="grid-ritem" onclick="removeRecord(event)"><i
                                        class="fas fa-minus minus-btn"></i>
                                </div>
                                <div class="grid-ritem">
                                    <input type="text" name="used_framework[{{ $index }}]"
                                        list="used_framework" class="skill-input form-control" placeholder="言語を入力"
                                        value="{{ $used_data->name }}">
                                    <datalist id="used_framework">
                                        @foreach ($langs as $data)
                                            <option value="{{ $data->name }}">
                ...
                <div id="database" class="grid-container card w-100">
                    ...
                    <div class="grid-body px-2 ">
                        ...
                        @foreach ($used_databases as $index => $used_data)
                            <div class="grid-record" data-recordid="{{ $index }}">
                                <div class="grid-ritem" onclick="removeRecord(event)"><i
                                        class="fas fa-minus minus-btn"></i>
                                </div>
                                <div class="grid-ritem">
                                    <input type="text" name="used_database[{{ $index }}]" list="used_database"
                                        class="skill-input form-control" placeholder="言語を入力"
                                        value="{{ $used_data->name }}">
                                    <datalist id="used_database">
                                        @foreach ($langs as $data)
            ...
            <div class="inner-container">
                ...
                <div id="middleware" class="grid-container card w-100">
                    ...
                    <div class="grid-body px-2 ">
                        ...
                        @foreach ($used_middlewares as $index => $used_data)
                            <div class="grid-record" data-recordid="{{ $index }}">
                                <div class="grid-ritem" onclick="removeRecord(event)"><i
                                        class="fas fa-minus minus-btn"></i>
                                </div>
                                <div class="grid-ritem">
                                    <input type="text" name="used_middleware[{{ $index }}]"
                                        list="used_middleware" class="skill-input form-control" placeholder="言語を入力"
                                        value="{{ $used_data->name }}">
                                    <datalist id="used_middleware">
                                        @foreach ($langs as $data)
                                            <option value="{{ $data->name }}">
                ...
                <div id="os" class="grid-container card w-100">
                    ...
                    <div class="grid-body px-2 ">
                        ...
                        @foreach ($used_oses as $index => $used_data)
                            <div class="grid-record" data-recordid="{{ $index }}">
                                <div class="grid-ritem" onclick="removeRecord(event)"><i
                                        class="fas fa-minus minus-btn"></i>
                                </div>
                                <div class="grid-ritem">
                                    <input type="text" name="used_os[{{ $index }}]" list="used_os"
                                        class="skill-input form-control" placeholder="言語を入力"
                                        value="{{ $used_data->name }}">
                                    <datalist id="used_os">
                                        @foreach ($langs as $data)
                ...
                <div id="server" class="grid-container card w-100">
                    ...
                    <div class="grid-body px-2 ">
                        ...
                        @foreach ($used_servers as $index => $used_data)
                            <div class="grid-record" data-recordid="{{ $index }}">
                                <div class="grid-ritem" onclick="removeRecord(event)"><i
                                        class="fas fa-minus minus-btn"></i>
                                </div>
                                <div class="grid-ritem">
                                    <input type="text" name="used_server[{{ $index }}]" list="used_server"
                                        class="skill-input form-control" placeholder="言語を入力"
                                        value="{{ $used_data->name }}">
                                    <datalist id="used_server">
                                        @foreach ($langs as $data)
                ...
                <div id="version_management" class="grid-container card w-100">
                    ...
                    <div class="grid-body px-2">
                        ...
                        @foreach ($used_version_management as $index => $used_data)
                            <div class="grid-record" data-recordid="{{ $index }}">
                                <div class="grid-ritem" onclick="removeRecord(event)"><i
                                        class="fas fa-minus minus-btn"></i>
                                </div>
                                <div class="grid-ritem">
                                    <input type="text" name="used_version_management[{{ $index }}]"
                                        list="used_version_management" class="skill-input form-control"
                                        placeholder="言語を入力" value="{{ $used_data->name }}">
                                    <datalist id="used_version_management">
                                        @foreach ($langs as $data)
                                            <option value="{{ $data->name }}">
                                        @endforeach
